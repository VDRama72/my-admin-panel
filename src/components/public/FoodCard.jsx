...
export default function FoodCard({ food, onAddToCart }) {
  return (
    <div className="bg-white border rounded-md shadow-sm hover:shadow-md transition-all overflow-hidden w-full max-w-[140px] sm:max-w-[180px] text-left">
      <div className="w-full aspect-[4/3] bg-gray-100 overflow-hidden">
        <img
          src={
            food.image
              ? `${baseURL}${food.image.startsWith('/') ? '' : '/'}${food.image}`
              : 'https://via.placeholder.com/140x105?text=No+Image'
          }
          alt={food.name}
          className="w-full h-full object-cover"
          onError={(e) => {
            e.target.onerror = null;
            e.target.src = 'https://via.placeholder.com/140x105?text=No+Image';
          }}
        />
      </div>
